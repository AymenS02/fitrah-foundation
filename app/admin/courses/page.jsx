  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-5xl mx-auto p-6">
        <h1 className="text-3xl text-foreground font-bold mb-6 text-center font-palanquin-dark">
          Available Courses
        </h1>
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-card shadow-md rounded-xl p-6 mb-6 border border-border">
            ...
            <textarea 
              ...
              className="border border-border bg-background text-foreground p-2 rounded w-full mt-4 focus:border-primary focus:outline-none" 
            />
            <textarea 
              name="requirements" 
            ...
            <button 
              type="submit" 
              className="bg-success text-white px-4 py-2 rounded-lg mt-4 hover:opacity-90 transition-opacity"
            >
              Save Course
            </button>
          </form>
        ...
        {/* Courses List */}
        {courses.length === 0 ? (
          <p className="text-center mt-10 text-muted-foreground">No courses available.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => (
              <div 
                key={course._id} 
                className="bg-card shadow-md rounded-xl p-5 border border-border hover:shadow-lg transition-shadow"
              >
                {course.thumbnailUrl && (
                  <img 
                ...
                <p className="text-sm text-primary mt-2">Category: {course.category || 'Uncategorized'}</p>
                <p className="text-sm text-muted-foreground">Level: {course.difficultyLevel}</p>
                <p className="text-sm mt-1 text-foreground font-medium">Price: ${course.price}</p>
