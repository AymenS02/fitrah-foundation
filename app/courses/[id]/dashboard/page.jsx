...
import { CheckCircle, Clock, BookOpen, FileText, ClipboardList, Play, Lock } from "lucide-react";
...
export default function CourseDashboardPage() {
  ...
  // Check if module is accessible (previous modules completed or it's the first)
  const isModuleAccessible = (moduleIndex) => {
    if (moduleIndex === 0) return true;
    const modules = course?.modules || [];
    const previousModule = modules[moduleIndex - 1];
    return previousModule ? isModuleCompleted(previousModule._id) : false;
  };
  ...
  const handleModuleClick = (module, moduleIndex) => {
    ...
    // Navigate to module view or complete simple modules
    if (module.type === 'text' || module.type === 'pdf') {
      // For simple content, can mark as completed immediately
          ...
          {modules.length === 0 ? (
            ...
            modules.map((module, index) => {
              ...
              return (
                <div
                  key={module._id}
                  className={`bg-card rounded-lg shadow-sm border-l-4 transition-all duration-200 ${
                    isCompleted
                      ? 'border-green-500 bg-green-50'
                      : isAccessible
                      ? 'border-blue-500 hover:shadow-md cursor-pointer'
                ...
                >
                  ...
                  >
                    ...
                    {isAccessible && !isCompleted && (module.type === 'text' || module.type === 'pdf') && (
