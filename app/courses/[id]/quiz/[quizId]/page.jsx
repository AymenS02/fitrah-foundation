      ...
      {!loading && !user && (
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-2 font-palanquin-dark">Not Logged In</h2>
            <p className="text-muted-foreground mb-4">You must be logged in to take this quiz.</p>
            <button
      ...
      {!loading && error && !quizData && (
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-2 font-palanquin-dark">Error</h2>
      ...
      {!loading && !quizData && !error && (
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-2 font-palanquin-dark">No Quiz Found</h2>
      ...
      {!loading && submitted && score !== null && quizData && (
        <div className="container mx-auto max-w-4xl px-4">
          <div className="bg-card rounded-lg shadow-lg border border-border p-8 text-center">
            ...
            <h1 className="text-3xl font-bold text-foreground mb-2 font-palanquin-dark">
            ...
            <div className="bg-gradient-to-br from-muted to-background rounded-lg p-6 mb-6 border border-border">
              <div className="text-5xl font-bold text-primary mb-2">{score.toFixed(0)}%</div>
      ...
      {!loading && !submitted && quizData && (
        <div className="container mx-auto max-w-4xl px-4">
          ...
          <div className="bg-card rounded-lg shadow-sm border border-border p-6 mb-6">
            ...
            <h1 className="text-3xl font-bold text-foreground mb-2 font-palanquin-dark">{quizData.title}</h1>
            {quizData.description && <p className="text-muted-foreground mb-4">{quizData.description}</p>}
            <div className="flex items-center space-x-6 text-sm text-muted-foreground bg-muted rounded-lg p-4">
              ...
              <div>
                Passing score: <span className="font-semibold text-foreground">{passingScore}%</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6 mb-6">
            {questions.map((q, idx) => (
              <div key={idx} className="bg-card rounded-lg shadow-sm p-6 border border-border hover:border-primary/50 transition-colors">
                <h3 className="text-lg font-semibold text-foreground mb-4">
                  <span className="inline-block w-8 h-8 bg-primary text-white rounded-full text-center leading-8 mr-3 text-sm font-bold">
                    {idx + 1}
                  </span>
                  {q.question}
                </h3>
                <div className="space-y-3 ml-11">
                  {q.options.map((opt, optIdx) => (
                    <label
                      key={optIdx}
                      className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all active:scale-[0.98] ${
                        answers[idx] === opt
                    ...
                    >
                      <input
                        ...
                        className="hidden"
                      />
                      <span className={`${answers[idx] === opt ? "font-medium" : "text-foreground"}`}>
                        {opt}
                      </span>
                    </label>
                  ))}
          ...
          {/* Submit */}
          <div className="bg-card rounded-lg shadow-sm border border-border p-6 sticky bottom-4">
            <div className="flex items-center justify-between">
              <div className="text-sm">
                <span className="text-muted-foreground">Progress: </span>
                <span className={`font-semibold ${allQuestionsAnswered() ? "text-success" : "text-primary"}`}>
              ...
              <button
                ...
                className="px-8 py-3 bg-primary text-white rounded-lg hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed font-semibold shadow-md transition-all active:scale-95"
              >
